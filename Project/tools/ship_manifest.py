...
official bytes, immutable evidence bytes, and an eligible decision from the
shared audit authority.

Regenerate at freeze, AFTER the final commit. The working tree and every
selected input must already be committed. The output is
...
def prospective_audit_packet(reference: Any, entry_id: str,
    ...
    try:
        bound = load_bound_packet(entry_id, packet_sha256=packet_sha)
    except AuditAuthorityError as exc:
...
def journal_submission_sha(row: dict[str, Any]) -> str | None:
    ...
    if isinstance(explicit, str):
        return explicit
    impl = row.get("impl", {})
    if impl.get("path") == SUBMISSION_REL:
        return impl.get("sha256")
    return None


def build_journal_entry(selector: dict[str, Any], shape_id: int,
                        expected_shape: dict[str, Any], submission_sha: str,
                        rows: dict[str, dict[str, Any]]) -> dict[str, Any]:
    if set(selector) - {"kind", "entry_id", "selection_rationale"}:
        raise ManifestError(f"shape {shape_id}: unknown journal selector fields")
    entry_id = selector.get("entry_id")
    row = rows.get(entry_id)
    if row is None:
        raise ManifestError(f"shape {shape_id}: journal entry is absent: {entry_id}")
    if row.get("type") != "candidate" or row.get("shape_id") != shape_id:
        raise ManifestError(f"shape {shape_id}: selected row is not its candidate result")
    exact_shape(row.get("shape"), expected_shape, f"journal entry {entry_id}")
    if row.get("correctness", {}).get("passed") is not True:
        raise ManifestError(f"shape {shape_id}: selected journal result did not pass")
    measured_submission_sha = journal_submission_sha(row)
    if measured_submission_sha != submission_sha:
        raise ManifestError(
            f"shape {shape_id}: journal result is not bound to final submission "
            f"({measured_submission_sha!r} != {submission_sha})"
        )
    try:
        bound_packet = load_bound_packet(entry_id)
    except AuditAuthorityError as exc:
        raise ManifestError(f"shape {shape_id}: audit packet is invalid: {exc}") from exc
    if bound_packet.candidate_sha256 != submission_sha:
        raise ManifestError(f"shape {shape_id}: audit packet contains different bytes")
    audit = eligible_audit(entry_id, submission_sha, bound_packet.sha256)
    timing = row.get("timing", {})
    candidate_timing = timing.get("candidate", {})
    median_ms = require_positive_number(
        candidate_timing.get("median_ms"), f"shape {shape_id} candidate median"
    )
    return {
        "evidence_kind": "trusted-controller-journal",
        "evidence_path": str(JOURNAL.relative_to(ROOT)),
        "entry_id": entry_id,
        "audit_packet_path": str(bound_packet.path.relative_to(ROOT)),
        "audit_packet_sha256": bound_packet.sha256,
        "candidate": row.get("impl"),
        "submission_sha256": submission_sha,
        "correctness": row["correctness"],
        "timing": timing,
        "median_ms": median_ms,
        "reference_method": "paired pinned-official dense reference in trusted controller",
        "audit_authority": audit,
        "selection_rationale": selector.get("selection_rationale", "owner final evidence map"),
    }
...
def build_controller_entry(selector: dict[str, Any], shape_id: int,
    ...
    if timing_args != {"warmup": 20, "repeats": 100, "rounds": 3}:
        raise ManifestError(f"shape {shape_id}: timing protocol is not official")
    ...
    return {
        ...
        "audit_authority": audit,
        "selection_rationale": selector.get(
            "selection_rationale", "owner final evidence map"
        ),
    }
...
def require_shape14_protocol(packet: dict[str, Any]) -> None:
    ...
    if (
        ...
        or len(matrix["values"]) != 32
    ):
        raise ManifestError("shape 14 per-slice timing matrix is malformed")
    by_batch = [
    ...
    for repeat in range(repeats):
        expected_sum = sum(by_batch[batch][repeat] for batch in range(32))
...
def build_side_controller_entry(
    ...
    allowed = {
        "kind", "entry_id", "measurement_event_sha256",
        "side_evidence_sha256", "audit_packet", "selection_rationale",
    }
    if set(selector) - allowed or shape_id not in {6, 14}:
    ...
    if (
        ...
        or [item.get("stage") for item in stage_refs if isinstance(item, dict)]
        != expected_stages
    ...
    if shape_id == 6:
        ...
        reference_method = (
            "batch-chunked pinned official dense baseline; batch-independent exact computation"
        )
    else:
        if (
        ...
        require_shape14_protocol(final_packet)
        median_ms = final_packet.get("timing", {}).get(
            "gpu_compute_median_of_sums_ms"
        )
        reference_method = (
            "validated streamed fp32 oracle; 32 serial B=1 slices, not literal B=32"
        )
...
def validate_evidence_map(payload: dict[str, Any], submission_sha: str,
    ...
    for shape_id, selector in selectors.items():
        if not isinstance(selector, dict) or selector.get("kind") not in {
            "controller", "journal", "side_controller"
        }:
...
def build_manifest(evidence_map_path: Path, head: str,
    ...
    for shape_id in sorted(expected_shapes):
        ...
        if selector["kind"] == "controller":
            selection = build_controller_entry(
                selector, shape_id, expected_shapes[shape_id], submission_sha
            )
        elif selector["kind"] == "journal":
            selection = build_journal_entry(
                selector, shape_id, expected_shapes[shape_id], submission_sha, rows
            )
        else:
    ...
    return {
        "schema_version": "ship-manifest-v2",
...
def main() -> int:
    ...
    try:
        ...
        print(f"{output} — 14 explicitly selected, audit-eligible shapes")
        return 0
