...
| [host-sync-in-timed-region.md](host-sync-in-timed-region.md) | CUDA-event timing does NOT make host work free — it makes it free only while the host stays ahead. `bool(mask.all())` in forward() synced every iteration, and the device-idle gap between the drain and the next `cudaGraphLaunch` lands inside the measured window. Shape 2 host work 91 -> 12.6 us/iter against 64.7 us of device work, i.e. the loop was host-BOUND; the four kernels moved <=2.5% (the control). "Self CUDA total" cannot show this — the win is in a gap no kernel row reports, and only a runner measurement can size it |
