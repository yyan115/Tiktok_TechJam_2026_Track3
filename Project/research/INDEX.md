...
| [head-count-scaling.md](head-count-scaling.md) | Shapes 1/9/10/11 are the same 7.52 GFLOP problem at 1/2/4/16 heads: the BASELINE degrades 4.3x with head count (so the per-shape speedup spread is mostly a baseline property), our kernel is flat to 4 heads then pays +63.7% at 16 — and head_dim-8 padding bounds at 14.3%, so it explains at most a fifth of that; the rest is open |
