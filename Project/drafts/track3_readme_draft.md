...
- The shape-6 and shape-14 evidence packets are provisional — one seed
  each, and they cite the pre-integration submission file. We attempted the
  re-capture against the shipped file and it is **blocked by a one-line bug
  in our own evidence tooling**: both evaluators compare a mask's device
  against `torch.device("cuda")` while the official generator returns it on
  `cuda:0`, and those compare unequal in PyTorch. The fix is owner-only —
  the optimizing agent cannot write to the tools that decide whether its own
  claims are true. Until then these packets cannot be regenerated by the
  tools in this repo, and we say so rather than implying a re-capture is
  merely pending.
- Sub-millisecond shapes are genuinely noisy on a consumer card, and we now
  have three separate measurements of how noisy, which are worth keeping
  apart: *within* one invocation, baseline-vs-itself calibration noise is
  **0.03–0.4%** (that is what sets each shape's promotion threshold);
  *across* invocations of identical work, GPU clock state alone moves
  absolute time by about **9%**; and measuring the shipped file against its
  own kernel module in separate invocations gave **−10.0% to +2.6%** per
  shape. The older "±25%" figure came from comparing two *uncontrolled*
  pre-gate boards and should not be quoted for these. Each published row is
  the **median of 300 paired samples inside a single invocation**, not an
  average across sweeps — we deliberately never average across invocations,
  because absolute latencies are not comparable across processes.
