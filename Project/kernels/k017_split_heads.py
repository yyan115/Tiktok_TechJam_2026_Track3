"""k017: split the megakernel's head loop into its own kernel.

Two measured problems, one change.

PROBLEM 1 — grid starvation on the small-batch shapes. k009's K2 is launched
on grid (q_tiles, B). Shape 2 is batch 1, seq 128, and Triton's tl.dot needs
BLOCK_M >= 16, so the grid floor is (8, 1) = 8 CTAs on a 38-SM card: 79% of
the GPU idle. Shape 3 (batch 4) gets 32. Their measured MFU is 0.03 and 0.10,
the two worst on the board, and the roofline note attributes that to "a grid
too small to fill 38 SMs" rather than to bandwidth.

PROBLEM 2 — the out-projection pays the tl.dot padding H times. k009 folds it
into the head loop as H separate [BLOCK_M, HD_PAD] x [HD_PAD, D] dots. When
HD < 16 (shapes 7 and 11) HD_PAD is 16, so every one of those H dots runs at
half useful width. Measured on shape 11: K2 is 2.13x slower than the identical
kernel at HD=32 while every other kernel is flat within 4%, and widening the
autotune space (k016) recovered only 3.7% of it against a same-session
control — so the residue is the padded arithmetic, not tiling.

THE CHANGE. Split K2 into two kernels:

  K2a `_attn_heads`   grid (q_tiles, B, H) — one CTA per head. Each runs the
                      causal flash attention for its own head and writes the
                      normalised head output into a context buffer at its own
                      column slice. Heads are independent, so there is no
                      cross-CTA communication and no atomics: each CTA owns a
                      disjoint slice of the buffer.
  K2b `_block_tail`   grid (q_tiles, B) — loads the assembled context tile
                      [BLOCK_M, D] and does out-proj + residual + norm2 +
                      FFN + residual exactly as k009 does.

That buys H times the parallelism on the attention half (shape 2: 8 -> 32
CTAs, shape 3: 32 -> 128), and it turns H padded out-projection dots into ONE
[BLOCK_M, D_PAD] x [D_PAD, D] dot at full width.

WHAT IT COSTS. One extra kernel per layer and one round trip of the context
buffer ([B, S, D] fp16) through L2/HBM. Inside the whole-forward CUDA graph a
launch is cheap, and the context buffer is 32 KB on shape 2, but this is a
real cost and it is why this file is measured against k009 on the same box
rather than assumed to win.

MATH IS UNCHANGED. Same flash-attention recipe, same online softmax, same
fp32 residual stream and LayerNorm statistics, same fp16 dots with fp32
accumulation, same exact-erf GELU, same eager fallback for padded masks.
The only difference is where the head results are materialised.
...
NAME = "k017_split_heads"
DESCRIPTION = ("megakernel with the head loop split into its own kernel: H times "
               "the attention parallelism, and one full-width out-projection.")
...
def _attn_heads(
    ...
    """One CTA per (q-tile, batch, head): causal flash attention for that head,
    written straight into its own column slice of the context buffer.

    Heads never interact here, so the H-way split needs no atomics and no
...
def _block_tail(
    ...
    """out-proj + residual + norm2 + FFN(erf-GELU) + residual.

    The out-projection is now ONE [BLOCK_M, D_PAD] x [D_PAD, D] dot over the
    assembled context, at full width, instead of H dots of contraction HD_PAD
    — which is where the head_dim-8 shapes were losing half of every dot.
    """
    pid_m = tl.program_id(0)
    pid_b = tl.program_id(1)
...
def _pack_layer(layer):
    cache = getattr(layer, "_k017_cache", None)
    if cache is None:
        ...
        object.__setattr__(layer, "_k017_cache", cache)
    return cache
...
def _fused_forward(model, x):
    ...
    bufs = getattr(model, "_k017_buf", None)
    if bufs is None or bufs["a"].shape != x.shape:
        ...
        object.__setattr__(model, "_k017_buf", bufs)
    qkv = bufs["qkv"]
    ctx = bufs["ctx"]
...
def build(otb, config):
    ...
    class SplitHeadTransformer(otb.UserOptimizedTransformer):
    ...
    model.__class__ = SplitHeadTransformer
    return model
