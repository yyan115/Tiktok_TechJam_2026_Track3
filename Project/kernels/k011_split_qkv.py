"""k011: k009 with the norm+QKV kernel parallelized across weight chunks.

Profiling shape 9 (29 Aug) showed _norm_qkv at 2.3x its memory floor with
only 64 programs in flight; moving the three q/k/v weight chunks to a second
grid axis triples the program count. LayerNorm is recomputed per chunk
(cheap). Everything else is k009 unchanged; original header follows.
...
NAME = "k011_split_qkv"
DESCRIPTION = ("k009 megakernel with the norm+QKV kernel's weight chunks "
               "split onto a second grid axis (3x programs — occupancy fix "
               "found by profiling shape 9).")
...
def _norm_qkv(
    ...
    k011 change: the q/k/v weight chunks live on a second grid axis instead
    of a static in-program loop — 3x the programs, so small-token shapes
    keep the SMs fed. The (cheap) LayerNorm is recomputed per chunk."""
    pid = tl.program_id(0)
