...
@triton.autotune(
    configs=[
        # num_stages > 1 multiplies the shared-memory cost of the loads inside
        # the loop, and three 32 KB weight tiles leave very little headroom
        # under the 101,376-byte limit. Stage counts are kept low and BLOCK_M
        # capped at 64 for that reason, not by preference.
...
def _block_tail(
    ...
    # The weight tiles are loop-invariant and hoisting them out looks free. It
    # is not: wo, wf1 and wf2 are 32 KB each at D = FFN = 128, so holding all
    # three across the loop costs 96 KB before pipelining, and the first version
    # of this kernel failed to compile at 114,688 bytes against a 101,376 limit.
    # They are loaded inside the loop instead and left to L2, which is the right
    # cache for a 96 KB working set read identically by every program.
