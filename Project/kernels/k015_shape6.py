...
NAME = "k015_shape6"
DESCRIPTION = ("Shape-6 candidate: k009 megakernel with CUDA graphs DISABLED "
               "above 256k tokens (capture memory risk; launch savings "
               "negligible at B=10000) — Card C2.")
...
def build(otb, config):
    ...
    class FusedBlockTransformer(otb.UserOptimizedTransformer):
        ...
        def forward(self, x, valid_token_mask=None):
            ...
            if x.shape[0] * x.shape[1] >= 262144:
                # Huge-token route (shape 6): no graph capture — its memory
                # cost is unproven at this size and launch overhead is
                # negligible when every kernel runs for milliseconds.
