...
#define D_MAX 128
#define LN_EPS 1e-5f
// Shared budget, kept under the 48 KB static limit so no opt-in is needed:
//   Wsh  D_MAX*D_MAX  fp16 = 32 KB   weight chunk, staged ONCE per block
//   Ysh  BLOCK_T*D_MAX fp16 =  8 KB  LayerNormed tile
//   Osh  WARPS*16*16  fp32 =  2 KB   wmma epilogue staging
//                            42 KB   -> two blocks per SM (100 KB available)
...
__global__ __launch_bounds__(WARPS * WARP) void norm_qkv_kernel(
    ...
    const int tokens,
    const int D) {
  __shared__ __half Wsh[D_MAX * D_MAX];        // weight chunk, staged once
  __shared__ __half Ysh[BLOCK_T * D_MAX];      // LayerNormed tile
  ...
  // Stage the whole weight chunk into shared ONCE per block. It is constant
  // across every token tile this block strides over, so re-reading it from
  // global inside the tile loop -- which the first version of this kernel did,
  // and which cost it 203 us against Triton's 49 -- is pure waste. Layout is
  // kept as [n][k] so the wmma B fragment reads it column-major with ldm = D.
  for (int i = tid; i < D * D; i += WARPS * WARP) Wsh[i] = Wc[i];
  __syncthreads();

  // ---- normalise one token tile into Ysh
  auto stage = [&](int tile) {
  ...
  // ---- GEMM the staged tile out of Ysh
  auto emit = [&](int tile) {
    ...
    for (int n0 = 0; n0 < D; n0 += 16) {
      ...
      for (int k0 = 0; k0 < D; k0 += 16) {
        wmma::load_matrix_sync(a, Ysh + row0 * D_MAX + k0, D_MAX);
        // Wsh is [n][k]; as a [K,N] matrix that is column-major with ldm = D.
        wmma::load_matrix_sync(b, Wsh + n0 * D + k0, D);
  ...
  // Grid-stride over token tiles with the weight chunk resident in shared.
  // The LN/GEMM double buffer of the first version is gone: it cost 8 KB of the
  // shared budget that the weight chunk needs far more, and staging the weights
  // is worth an order of magnitude more than overlapping the prologue.
