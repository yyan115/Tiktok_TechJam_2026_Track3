...
def _load_cuda():
    """Compile once. Any failure silently leaves the Triton path in place."""
    ...
    try:
        ...
        for ccbin in ("g++-15", None):
            try:
                flags = cuda_flags + ([f"-ccbin={ccbin}"] if ccbin else [])
                _CUDA_MOD = load_inline(
                    name=f"k023_norm_qkv_{ccbin or 'default'}".replace("+", "p"),
                ...
                _CUDA_ERROR = f"{type(exc).__name__}: {str(exc)[:400]}"
                continue
        _CUDA_OK = False
    except Exception as exc:
        _CUDA_ERROR = f"{type(exc).__name__}: {str(exc)[:400]}"
